...
const trip_detail = ({ loaderData }: Route.ComponentProps) => {
        ...
        <section className="container wrapper:md">
            <header>
                ...
                <div className="flex items-center gap-5">
                    ...
                    <InfoPill text={`${itinerary?.slice(0,4).map((item) => item.location).join(', ') || ""}`} image="/assets/icons/location-mark.svg"/>
                </div>
            </header>
            <section className="gallery">
                {imageUrls.map((image: string, i: number) => (
                    <img src={image} key={i} alt="Picture related to the trip" className={cn("w-full rounded-xl object-cover", i === 0 ? 'md:col-span-2 md:row-span-2 h-[350px]' : 'md:row-span-1 h-[150px]')} />
                ))}
            </section>
            <section className="flex gap-3 md:gap-5 items-center flex-wrap">
                ...
                <ul className="flex gap-1 items-center">
                        {Array(5).fill('null').map((value, i) => (
                            <li key={i}><img src="/assets/icons/star.svg" alt="Star" className="size-[18px]"/></li>
                        ))}
                        <li className="ml-1">
                            <ChipListComponent>
                                <ChipsDirective>
